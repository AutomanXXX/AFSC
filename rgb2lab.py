...
def lab_loss_metric(img1,img2):
    b, c, h, w = img1.shape
    Results=0
    for i in range(b):
        img_1 = img1[i].unsqueeze(dim=0)
        lab_1 = xyz2lab(rgb2xyz(img_1))
        img_2 = img2[i].unsqueeze(dim=0)
        lab_2 = xyz2lab(rgb2xyz(img_2))
        diff=(lab_1-lab_2)*(lab_1-lab_2)
        RD=diff[0,1,:,:]
        BD=diff[0,2,:,:]
        Result=RD+BD
        Results = Results + Result.mean()
    return Results


# lab = xyz2lab(rgb2xyz(img))  #img:b*c*h*w  RGB