...
def test(obj_names, mvtec_path, checkpoint_path, run_name_top, which_epoch_model):
    ...
    for obj_name in obj_names:
        ...
        model.load_state_dict(torch.load(os.path.join(checkpoint_path, save_name+".pckl"), map_location='cuda:0'),strict=False)
        model.cuda()
        model.eval()
        ...
        anomaly_mse_mean_all=0
        normal_mse_mean_all=0
        anomaly_ssim_mean_all=0
        normal_ssim_mean_all=0
        num_none_anomaly=0
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                num_anomaly_mask = torch.sum(true_mask)
                normal_mask = 1 - true_mask
                num_normal_mask = torch.sum(normal_mask)
                ...
                total_gt_pixel_scores[mask_cnt * img_dim1 * img_dim2:(mask_cnt + 1) * img_dim1 * img_dim2] = flat_true_mask
                mask_cnt += 1
                ...
                for i in range(out_mask_averaged.shape[0]):
                    if is_normal and true_mask[i].max()==1:
                        aupro_list.append(compute_pro(true_mask[i].cpu().numpy().astype(int), out_mask_averaged[i,0,...].squeeze()[np.newaxis,:,:]))

        anomaly_mse_mean_all = anomaly_mse_mean_all / (len(dataset) - num_none_anomaly)
        normal_mse_mean_all = normal_mse_mean_all / len(dataset)
        anomaly_ssim_mean_all = anomaly_ssim_mean_all / (len(dataset) - num_none_anomaly)
        normal_ssim_mean_all = normal_ssim_mean_all / len(dataset)
...
if __name__=="__main__":
    ...
    parser.add_argument('--data_path', action='store', type=str, default=r'./mvtec_anomaly_detection/')
